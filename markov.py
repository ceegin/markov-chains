...
def tweet():
    """Tweets the text generated by Markov algorithm to twitter using api"""
    # taking in all keys and secrets to authenticate API
    ivy_api = twitter.Api(
        consumer_key=os.environ['IVY_TWITTER_CONSUMER_KEY'],
        consumer_secret=os.environ['IVY_TWITTER_CONSUMER_SECRET'],
        access_token_key=os.environ['IVY_TWITTER_ACCESS_TOKEN_KEY'],
        access_token_secret=os.environ['IVY_TWITTER_ACCESS_TOKEN_SECRET'])

    cg_api = twitter.Api(
        consumer_key=os.environ['CG_TWITTER_CONSUMER_KEY'],
        consumer_secret=os.environ['CG_TWITTER_CONSUMER_SECRET'],
        access_token_key=os.environ['CG_TWITTER_ACCESS_TOKEN_KEY'],
        access_token_secret=os.environ['CG_TWITTER_ACCESS_TOKEN_SECRET'])

# printing to twitter
    message = play_markov()
    status = ivy_api.PostUpdate(message)
    status2 = cg_api.PostUpdate(message)
    print status.text
    tweet_again = raw_input("Enter to tweet again or q to quit. > ")
